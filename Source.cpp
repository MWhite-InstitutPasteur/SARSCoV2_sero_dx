...
#define N_mcmc 1000000        // Number of MCMC iterations: indexed by mc
...
#define N_data_cols 32        // Number of columns in data frame
#define N_part 215             // Number of participants to read in data from
#define N_negcon 335           // Number of negative control participants to read in data from
...
struct part_n
{
	...
	int group;                // 1 = Bichat; 2 = Strasbourg; 3 = Cochin; 4 = Thailand; 5 = Peru; 6 = EFS
...
int main(int argc, char** argv)
{
	...
	char* AB_negcon_input_File = argv[2];
	char* global_output_File   = argv[3];
	char* local_output_File    = argv[4];
	double long_half           = atof(argv[5]);
	...
	///////////////////////////////////////////////////////
	// 1.4 Read in antibody data (negative controls)

	std::ifstream AB_negcon_Stream(AB_negcon_input_File);

	if (AB_negcon_Stream.fail())
	{
		std::cout << "Failure reading in data." << endl;
	}

	vector<vector<double>> AB_negcon_data_read;

	AB_negcon_data_read.resize(N_negcon);
	for (int i = 0; i < N_negcon; i++)
	{
		AB_negcon_data_read[i].resize(4);
	}
	...
	for (int i = 0; i < N_negcon; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			AB_negcon_Stream >> AB_negcon_data_read[i][j];
		}
	}

	AB_negcon_Stream.close();


	//////////////////////////////////////////////////////
	// 1.5 Create global parameter objects
	...
	theta.AB_min = 1.95e-5;
	theta.AB_max = 0.02;

	theta.lAB_min = log(1.95e-5);
	theta.lAB_max = log(0.02);


	//////////////////////////////////////////////////////////
	// 1.6 Create individual-level objects for participant n
	...
	for (int n = 0; n<N_part; n++)
	{
		...
		for (int j = 0; j<N_t; j++)
		{
			if (AB_data_read[n][2 + j] > -0.5)
			{
				part[n].tt.push_back(AB_data_read[n][2 + j]);
				part[n].AB.push_back(AB_data_read[n][2 + N_t + j]);

				part[n].lAB.push_back(log(AB_data_read[n][2 + N_t + j]));
		...
		part[n].Ab_bg   = genunf(0.1*part[n].AB_min, 0.3*part[n].AB_min);
	...
	//////////////////////////////////////////////////////////
	// 1.6.2 Negative controls

	for (int n = N_part; n < N_part + N_negcon; n++)
	{
		part[n].group = AB_negcon_data_read[n - N_part][0];
		...
		part[n].tt.push_back(10.0);
		part[n].AB.push_back(AB_negcon_data_read[n - N_part][3]);

		part[n].lAB.push_back(log(AB_negcon_data_read[n - N_part][3]));

		part[n].AB_min = part[n].AB[0];
		part[n].AB_max = part[n].AB[0];

		part[n].lAB_min = log(part[n].AB[0]);
		part[n].lAB_max = log(part[n].AB[0]);
	...
	for (int mc = 1; mc<N_mcmc; mc++)
	{
		...
		for (int n = 0; n < (N_part + N_negcon); n++)
		{
			...
			if( part[n].group == 1 || part[n].group == 2 || part[n].group == 3)
			{
...
double data_like_n(part_n* p, params* theta)
{
	...
	if (p->group == 4 || p->group == 5 || p->group == 6)
	{
...
double local_prior(part_n* p)
{
	...
	if (p->lAb_bg > -2.302585) { return -LARGE; } // log(0.1) = -2.302585
